...
export function useUser(tableRef: Ref, treeRef: Ref) {
  ...
  function onChange({ row, index }) {
    ElMessageBox.confirm(
      ...
      .then(async () => {
        ...
        await toggleUserStatus(row.id, { status: row.status });
  ...
  function onbatchDel() {
    ...
    ElMessageBox.confirm(`确认删除选中的 ${curSelected.length} 个用户？`, "系统提示", {
      type: "warning"
    })
      .then(async () => {
        await Promise.all(curSelected.map(r => deleteUser(r.id)));
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate(async valid => {
          if (valid) {
            ...
            if (title === "新增") {
              await createUser({ ...payload, password: curData.password });
            } else {
    ...
    addDialog({
      ...
      beforeSure: done => {
        ruleFormRef.value.validate(async valid => {
          if (valid) {
            await resetUserPassword({ userId: row.id, password: pwdForm.newPwd });
