...
export function useRole(treeRef: Ref) {
  ...
  function handleDelete(row) {
    ElMessageBox.confirm(`确认删除角色 ${row.name}？`, "系统提示", { type: "warning" })
      .then(async () => {
    ...
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate(async valid => {
          if (valid) {
            const payload = { name: curData.name, code: curData.code, remark: curData.remark, status: 1 };
            if (title === "新增") {
              await createRole(payload);
            } else {
              await updateRole(row.id, payload);
            }
            chores();
          }
        });
  ...
  async function handleSave() {
    ...
    await assignRoleMenu({ roleId: id, menuIds: treeRef.value.getCheckedKeys() });
