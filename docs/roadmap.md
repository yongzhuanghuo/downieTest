...
> 已落地（提交 5fe87fd 起逐步完善）：检查更新走服务端 `.env` 六键，下载源已从 GitHub Releases 迁到阿里云 OSS（国内可达）。原设计存档如下。

**目标**：用户打开旧版软件 → 软件自动查服务器有无新版 → 有就弹窗提示更新。

**整体链路**：
1. 打包（macOS zip / Windows exe）
2. CI 打 tag 自动把固定名安装包覆盖上传 OSS `releases/`（`.github/workflows/build.yml`；包内嵌 ffmpeg 后超 100MB，Gitee Releases 放不下；OSS bucket `4kdownlebucket` 在上海区，只对 `releases/*` 前缀开匿名只读）
3. 后端记录「最新版本号 + 下载地址」
4. 客户端启动检查 → 弹窗 → 引导下载
