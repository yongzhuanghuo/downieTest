...
from app.schemas import GifProcessRequest, Md5ProcessRequest, TaskResponse, WatermarkProcessRequest
from app.modules.media.services import video_tools
...
from app.modules.media.routers.delogo import _uploads as _files
...
async def upload_image(file: UploadFile = File(...)):
    ...
    return {"image_id": img_id, "url": oss_url or f"/tmp/{dest.name}"}
...
async def watermark_process(req: WatermarkProcessRequest):
    ...
    task = await create_task("watermark")
    task.status = "running"
