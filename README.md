...
### 阶段 13：字幕功能（下载导出 + 提取内嵌）⏳ 待开始

- 下载视频时导出字幕（yt-dlp `--write-subs --sub-langs`）
- 上传视频提取内嵌字幕（FFmpeg `-map 0:s`）

### 阶段 14：去水印工具（时间轴 + 多段框选）⏳ 待开始

- 独立工具：上传视频 → 预览 → 时间轴 + 多段框选 → FFmpeg `delogo` 按时间段抹除
- 覆盖「水印中途换位置」场景（关键帧模型，多段 delogo + `enable='between(t,..)'`）

### 阶段 15：14 天试用 ⏳ 待开始

- 免费版改为 14 天试用，到期锁定下载、需充会员
- 服务端记录试用起始时间（`trial_devices` 表），清本地数据无法重置
