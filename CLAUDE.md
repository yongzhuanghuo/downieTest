...
4KDownle（4K全能视频下载器）— Flutter 桌面视频下载器（macOS / Windows），通过 yt-dlp + FFmpeg 子进程做解析、下载、合并，带会员授权系统（自建 Node 后端）。抖音不走 yt-dlp（其提取器未实现 `a_bogus` 签名、被抖音反爬打穿），改用内置 WebView 拦截 `aweme/detail` API 响应拿播放地址。详细结构和技术栈见 [README.md](README.md)（含开发计划阶段 1-21）。
