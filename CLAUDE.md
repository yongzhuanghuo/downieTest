...
| 统一服务端（授权 + 视频处理） | [services/api/CLAUDE.md](services/api/CLAUDE.md) |
| 管理后台前端 | [apps/admin-web/README.md](apps/admin-web/README.md) |
...
- **命令要在对应子目录执行**。`flutter` 命令在 `apps/desktop/` 下，`npm` 在 `apps/mobile/` 或 `apps/admin-web/` 下，`uvicorn` 在 `services/api/` 下。在仓库根跑 `flutter pub get` 会失败。
...
- `apps/mobile` / `apps/harmony-pc` **走服务端**：手机与鸿蒙沙箱起不了子进程，全部交给 `services/api` 的 media 模块。

同一个功能在两条路线上是**两套独立实现**，改一边不会自动同步到另一边。抖音解析尤其如此（详见 [docs/architecture.md](docs/architecture.md) 的「已知重复」一节）。

### 安全红线

- `services/api` 的 `/files` `/tmp` 静态目录默认**无鉴权**（`SIGNED_URLS` 关闭时）。公网前开签名 URL + 收紧 `ALLOWED_ORIGINS`，否则是开放下载代理。
